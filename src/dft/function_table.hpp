...
#define ONEAPI_MKL_DFT_BACKEND_SIGNATURES(EXT, PRECISION, DOMAIN, T_REAL, T_FORWARD, T_BACKWARD)  \
    void (*commit_##EXT)(oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc,                  \
                         sycl::queue & queue);                                                    \
    void (*compute_forward_buffer_inplace_##EXT)(                                                 \
        oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc,                                   \
    ...
    void (*compute_forward_buffer_outofplace_##EXT)(                                              \
        oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc, sycl::buffer<T_FORWARD, 1> & in,  \
        sycl::buffer<T_BACKWARD, 1> & out);                                                       \
    void (*compute_forward_buffer_outofplace_split_##EXT)(                                        \
        oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc, sycl::buffer<T_REAL, 1> & in_re,  \
    ...
    sycl::event (*compute_forward_usm_outofplace_##EXT)(                                          \
        oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc, T_FORWARD * in, T_BACKWARD * out, \
        const std::vector<sycl::event> &dependencies);                                            \
    ...
    void (*compute_backward_buffer_outofplace_##EXT)(                                             \
        oneapi::mkl::dft::descriptor<PRECISION, DOMAIN> & desc, sycl::buffer<T_BACKWARD, 1> & in, \
    ...
    ONEAPI_MKL_DFT_BACKEND_SIGNATURES(f, oneapi::mkl::dft::precision::SINGLE,
                                      ...
                                      std::complex<float>)
    ONEAPI_MKL_DFT_BACKEND_SIGNATURES(c, oneapi::mkl::dft::precision::SINGLE,
                                      oneapi::mkl::dft::domain::COMPLEX, float, std::complex<float>,
                                      std::complex<float>)
    ONEAPI_MKL_DFT_BACKEND_SIGNATURES(d, oneapi::mkl::dft::precision::DOUBLE,
                                      oneapi::mkl::dft::domain::REAL, double, double,
                                      std::complex<double>)
    ONEAPI_MKL_DFT_BACKEND_SIGNATURES(z, oneapi::mkl::dft::precision::DOUBLE,
