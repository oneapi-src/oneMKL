...
sycl::event release_matrix_handle(sycl::queue &queue, oneapi::mkl::sparse::matrix_handle_t *handle,
...
#define DEFINE_SET_CSR_DATA(FP_TYPE, FP_SUFFIX, INT_TYPE, INT_SUFFIX)                             \
    template <>                                                                                   \
    void set_csr_data(sycl::queue &queue, oneapi::mkl::sparse::matrix_handle_t handle,            \
    ...
    template <>                                                                                   \
    sycl::event set_csr_data(sycl::queue &queue, oneapi::mkl::sparse::matrix_handle_t handle,     \
                             INT_TYPE num_rows, INT_TYPE num_cols, oneapi::mkl::index_base index, \
...
#define DEFINE_GEMV(FP_TYPE, FP_SUFFIX)                                                           \
    template <>                                                                                   \
    void gemv(sycl::queue &queue, oneapi::mkl::transpose transpose_val, const FP_TYPE alpha,      \
    ...
    template <>                                                                                   \
    sycl::event gemv(sycl::queue &queue, oneapi::mkl::transpose transpose_val,                    \
                     const FP_TYPE alpha, oneapi::mkl::sparse::matrix_handle_t A_handle,          \
                     const FP_TYPE *x, const FP_TYPE beta, const FP_TYPE *y,                      \
                     const std::vector<sycl::event> &dependencies) {                              \
        auto libkey = get_device_id(queue);                                                       \
...
#define DEFINE_GEMVDOT(FP_TYPE, FP_SUFFIX)                                                       \
    template <>                                                                                  \
    void gemvdot(sycl::queue &queue, oneapi::mkl::transpose transpose_val, FP_TYPE alpha,        \
                 oneapi::mkl::sparse::matrix_handle_t A_handle, sycl::buffer<FP_TYPE, 1> &x,     \
    ...
    template <>                                                                                  \
    sycl::event gemvdot(sycl::queue &queue, oneapi::mkl::transpose transpose_val, FP_TYPE alpha, \
                        oneapi::mkl::sparse::matrix_handle_t A_handle, FP_TYPE *x, FP_TYPE beta, \
...
#define DEFINE_SYMV(FP_TYPE, FP_SUFFIX)                                                           \
    template <>                                                                                   \
    void symv(sycl::queue &queue, oneapi::mkl::uplo uplo_val, FP_TYPE alpha,                      \
              oneapi::mkl::sparse::matrix_handle_t A_handle, sycl::buffer<FP_TYPE, 1> &x,         \
    ...
    template <>                                                                                   \
    sycl::event symv(sycl::queue &queue, oneapi::mkl::uplo uplo_val, FP_TYPE alpha,               \
                     oneapi::mkl::sparse::matrix_handle_t A_handle, FP_TYPE *x, FP_TYPE beta,     \
...
#define DEFINE_TRMV(FP_TYPE, FP_SUFFIX)                                                           \
    template <>                                                                                   \
    void trmv(sycl::queue &queue, oneapi::mkl::uplo uplo_val,                                     \
              oneapi::mkl::transpose transpose_val, oneapi::mkl::diag diag_val, FP_TYPE alpha,    \
              oneapi::mkl::sparse::matrix_handle_t A_handle, sycl::buffer<FP_TYPE, 1> &x,         \
              FP_TYPE beta, sycl::buffer<FP_TYPE, 1> &y) {                                        \
        auto libkey = get_device_id(queue);                                                       \
    ...
    sycl::event trmv(sycl::queue &queue, oneapi::mkl::uplo uplo_val,                              \
                     oneapi::mkl::transpose transpose_val, oneapi::mkl::diag diag_val,            \
                     FP_TYPE alpha, oneapi::mkl::sparse::matrix_handle_t A_handle, FP_TYPE *x,    \
                     FP_TYPE beta, FP_TYPE *y, const std::vector<sycl::event> &dependencies) {    \
        auto libkey = get_device_id(queue);                                                       \
...
#define DEFINE_TRSV(FP_TYPE, FP_SUFFIX)                                                          \
    template <>                                                                                  \
    void trsv(sycl::queue &queue, oneapi::mkl::uplo uplo_val,                                    \
              oneapi::mkl::transpose transpose_val, oneapi::mkl::diag diag_val,                  \
              oneapi::mkl::sparse::matrix_handle_t A_handle, sycl::buffer<FP_TYPE, 1> &x,        \
    ...
    sycl::event trsv(sycl::queue &queue, oneapi::mkl::uplo uplo_val,                             \
                     oneapi::mkl::transpose transpose_val, oneapi::mkl::diag diag_val,           \
                     oneapi::mkl::sparse::matrix_handle_t A_handle, FP_TYPE *x, FP_TYPE *y,      \
                     const std::vector<sycl::event> &dependencies) {                             \
        auto libkey = get_device_id(queue);                                                      \
...
#define DEFINE_GEMM(FP_TYPE, FP_SUFFIX)                                                          \
    template <>                                                                                  \
    void gemm(sycl::queue &queue, oneapi::mkl::layout dense_matrix_layout,                       \
    ...
    template <>                                                                                  \
    sycl::event gemm(sycl::queue &queue, oneapi::mkl::layout dense_matrix_layout,                \
                     oneapi::mkl::transpose transpose_A, oneapi::mkl::transpose transpose_B,     \
                     const FP_TYPE alpha, oneapi::mkl::sparse::matrix_handle_t A_handle,         \
