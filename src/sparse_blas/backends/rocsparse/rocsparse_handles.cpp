...
void init_coo_matrix(sycl::queue &queue, oneapi::mkl::sparse::matrix_handle_t *p_smhandle,
    ...
    auto event = queue.submit([&](sycl::handler &cgh) {
        ...
        submit_host_task(cgh, queue, [=](RocsparseScopedContextHandler &sc) {
            ...
            *p_smhandle = new oneapi::mkl::sparse::matrix_handle(
                roc_smhandle, row_ind, col_ind, val, num_rows, num_cols, nnz, index);
        });
    });
    event.wait_and_throw();
}
...
void init_csr_matrix(sycl::queue &queue, oneapi::mkl::sparse::matrix_handle_t *p_smhandle,
    ...
    auto event = queue.submit([&](sycl::handler &cgh) {
        ...
        submit_host_task(cgh, queue, [=](RocsparseScopedContextHandler &sc) {
            ...
            *p_smhandle = new oneapi::mkl::sparse::matrix_handle(
                roc_smhandle, row_ptr, col_ind, val, num_rows, num_cols, nnz, index);
        });
    });
    event.wait_and_throw();
}
