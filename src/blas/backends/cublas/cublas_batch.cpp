...
inline void gemm_batch_impl(sycl::queue &queue, transpose transa,
                       transpose transb, int64_t m, int64_t n, int64_t k, Ts alpha,
                       sycl::buffer<Ta, 1> &a, int64_t lda, int64_t stride_a, sycl::buffer<Tb, 1> &b,
                       int64_t ldb, int64_t stride_b, Ts beta, sycl::buffer<Tc, 1> &c, int64_t ldc,
                       int64_t stride_c, int64_t batch_size) {
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_cublas_host_task(cgh, queue, [=](CublasScopedContextHandler &sc) {
            ...
            cublasStatus_t err;
            CUBLAS_ERROR_FUNC_T_SYNC("cublasGemmStridedBatchedEx", cublasGemmStridedBatchedEx, err, handle, get_cublas_operation(transa),
                                     get_cublas_operation(transb), m, n, k, &alpha,
                                     a_, get_cublas_datatype<cuTypeA>(), lda, stride_a, b_, get_cublas_datatype<cuTypeB>(), ldb, stride_b, &beta, c_, get_cublas_datatype<cuTypeC>(),
                                     ldc, stride_c, batch_size, get_cublas_datatype<cuTypeS>(), cublas_gemm_algo);
        });
    });
}
...
#define GEMM_STRIDED_BATCH_LAUNCHER(TYPE_A, TYPE_B, TYPE_C, TYPE_S)                                          \
    void gemm_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,  \
                    ...
                    int64_t batch_size) {                                                          \
        gemm_batch_impl<TYPE_A, TYPE_B, TYPE_C, TYPE_S>(queue, transa, transb, m, n, k, alpha, a, lda, \
...
GEMM_STRIDED_BATCH_LAUNCHER(std::complex<float>, std::complex<float>, std::complex<float>, std::complex<float>)
GEMM_STRIDED_BATCH_LAUNCHER(std::complex<double>, std::complex<double>, std::complex<double>, std::complex<double>)
...
template <typename Ta, typename Tb, typename Tc, typename Ts>
inline sycl::event gemm_batch_strided_usm_impl(sycl::queue &queue,
                              transpose transa, transpose transb, int64_t m, int64_t n, int64_t k,
                              Ts alpha, const Ta *a, int64_t lda, int64_t stride_a, const Tb *b,
                              int64_t ldb, int64_t stride_b, Ts beta, Tc *c, int64_t ldc,
                              int64_t stride_c, int64_t batch_size,
                              const std::vector<sycl::event> &dependencies) {
    using cuTypeA = typename CudaEquivalentType<Ta>::Type;
    using cuTypeB = typename CudaEquivalentType<Tb>::Type;
    ...
    auto done = queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_cublas_host_task(cgh, queue, [=](CublasScopedContextHandler &sc) {
            ...
            CUBLAS_ERROR_FUNC_T_SYNC("cublasGemmStridedBatchedEx", cublasGemmStridedBatchedEx, err, handle, get_cublas_operation(transa),
                                     get_cublas_operation(transb), m, n, k, &alpha,
                                     a, get_cublas_datatype<cuTypeA>(), lda, stride_a, b, get_cublas_datatype<cuTypeB>(), ldb, stride_b, &beta, c, get_cublas_datatype<cuTypeC>(),
                                     ldc, stride_c, batch_size, get_cublas_datatype<cuTypeS>(), cublas_gemm_algo);
        });
    });
    return done;
...
#define GEMM_STRIDED_BATCH_LAUNCHER_USM(TYPE_A, TYPE_B, TYPE_C, TYPE_S)          \
    sycl::event gemm_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m,     \
                           ...
                           TYPE_S beta, TYPE_C *c, int64_t ldc, int64_t stride_c, int64_t batch_size, \
                           const std::vector<sycl::event> &dependencies) {                        \
        return gemm_batch_strided_usm_impl(queue, transa, transb, m, n, k, alpha, \
...
GEMM_STRIDED_BATCH_LAUNCHER_USM(std::complex<float>, std::complex<float>, std::complex<float>, std::complex<float>)
GEMM_STRIDED_BATCH_LAUNCHER_USM(std::complex<double>, std::complex<double>, std::complex<double>, std::complex<double>)

#undef GEMM_STRIDED_BATCH_LAUNCHER_USM

template <typename Ta, typename Tb, typename Tc, typename Ts>
inline sycl::event gemm_batch_usm_impl(sycl::queue &queue,
                              transpose *transa, transpose *transb, int64_t *m, int64_t *n,
                              int64_t *k, Ts *alpha, const Ta **a, int64_t *lda, const Tb **b,
                              int64_t *ldb, Ts *beta, Tc **c, int64_t *ldc, int64_t group_count,
                              int64_t *group_size, const std::vector<sycl::event> &dependencies) {
    using cuTypeA = typename CudaEquivalentType<Ta>::Type;
    using cuTypeB = typename CudaEquivalentType<Tb>::Type;
    using cuTypeC = typename CudaEquivalentType<Tc>::Type;
        ...
        onemkl_cublas_host_task(cgh, queue, [=](CublasScopedContextHandler &sc) {
            ...
            for (int64_t i = 0; i < group_count; i++) {
                CUBLAS_ERROR_FUNC_T_SYNC(
                    "cublasGemmBatchedEx", cublasGemmBatchedEx, err, handle, get_cublas_operation(transa[i]),
                    get_cublas_operation(transb[i]), (int)m[i], (int)n[i], (int)k[i],
                    &alpha[i], (const void *const *)(a + offset), get_cublas_datatype<cuTypeA>(), (int)lda[i],
                    (const void *const *)(b + offset), get_cublas_datatype<cuTypeB>(), (int)ldb[i],
                    &beta[i], (void *const *)(c + offset), get_cublas_datatype<cuTypeC>(), (int)ldc[i], (int)group_size[i], get_cublas_datatype<cuTypeS>(), cublas_gemm_algo);
                offset += group_size[i];
            }
        });
    });
...
#define GEMM_BATCH_LAUNCHER_USM(TYPE_A, TYPE_B, TYPE_C, TYPE_S)     \
    sycl::event gemm_batch(sycl::queue &queue, transpose *transa, transpose *transb, int64_t *m,  \
                           ...
                           const std::vector<sycl::event> &dependencies) {                        \
         return gemm_batch_usm_impl(queue, transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc, group_count, group_size, dependencies); \
    }
...
GEMM_BATCH_LAUNCHER_USM(std::complex<float>, std::complex<float>, std::complex<float>, std::complex<float>)
GEMM_BATCH_LAUNCHER_USM(std::complex<double>, std::complex<double>, std::complex<double>, std::complex<double>)
