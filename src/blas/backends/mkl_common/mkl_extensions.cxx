...
void gemm_bias(sycl::queue &queue, transpose transa, transpose transb, offset offsetc, int64_t m,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::gemm_bias(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), detail::get_onemkl_offset(offsetc), m, n, k, alpha, a, lda, ao, b, ldb, bo,
                          beta, c, ldc, co));
}
...
void gemmt(sycl::queue &queue, uplo upper_lower, transpose transa, transpose transb, int64_t n,
           ...
           int64_t ldc) {
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::gemmt(queue, detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), n, k, alpha, a, lda, b, ldb, beta, c,
                      ldc));
}
...
void omatcopy(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::omatcopy(queue, detail::get_onemkl_transpose(trans), m, n, alpha, a, lda, b, ldb));
}
...
void imatcopy(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::imatcopy(queue, detail::get_onemkl_transpose(trans), m, n, alpha, ab, lda, ldb));
}
...
void omatadd(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::omatadd(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, alpha, a, lda, beta, b, ldb, c, ldc));
}
...
sycl::event gemm_bias(sycl::queue &queue, transpose transa, transpose transb, offset offsetc,
    ...
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemm_bias(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), detail::get_onemkl_offset(offsetc), m, n, k, alpha, a, lda, ao, b, ldb,
...
sycl::event gemmt(sycl::queue &queue, uplo upper_lower, transpose transa, transpose transb,
                  ...
                  const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemmt(queue, detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), n, k, alpha, a, lda, b, ldb, beta,
                             c, ldc, dependencies));
}
...
sycl::event omatcopy(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
                     ...
                     const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::omatcopy(queue, detail::get_onemkl_transpose(trans), m, n, alpha, a, lda, b, ldb, dependencies));
}
...
sycl::event imatcopy(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
                     ...
                     const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::imatcopy(queue, detail::get_onemkl_transpose(trans), m, n, alpha, ab, lda, ldb, dependencies));
}
...
sycl::event omatadd(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,
                    ...
                    const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::omatadd(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, alpha, a, lda, beta, b, ldb, c, ldc,
                               dependencies));
}
