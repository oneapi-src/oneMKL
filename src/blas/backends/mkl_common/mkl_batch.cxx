...
void gemv_batch(sycl::queue &queue, transpose transa, int64_t m, int64_t n, float alpha,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::gemv_batch(queue, detail::get_onemkl_transpose(transa), m, n, alpha, a, lda, stride_a, x, incx, stride_x, beta, y,
...
void dgmm_batch(sycl::queue &queue, side left_right, int64_t m, int64_t n,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::dgmm_batch(queue, detail::get_onemkl_side(left_right), m, n, a, lda, stride_a, x, incx, stride_x, c, ldc,
                           stride_c, batch_size));
}
...
void gemm_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::gemm_batch(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, k, alpha, a, lda, stride_a, b, ldb,
...
void trsm_batch(sycl::queue &queue, side left_right, uplo upper_lower, transpose trans,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::trsm_batch(queue, detail::get_onemkl_side(left_right), detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(trans), detail::get_onemkl_diag(unit_diag), m, n, alpha, a, lda,
...
void syrk_batch(sycl::queue &queue, uplo upper_lower, transpose trans, int64_t n, int64_t k,
                ...
                sycl::buffer<float, 1> &c, int64_t ldc, int64_t stride_c, int64_t batch_size) {
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::syrk_batch(queue, detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(trans), n, k, alpha, a, lda, stride_a, beta, c, ldc,
                           stride_c, batch_size));
}
...
void omatcopy_batch(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
    ...
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::omatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, a, lda, stride_a, b, ldb, stride_b,
                               batch_size));
}
...
void imatcopy_batch(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
                    ...
                    int64_t batch_size) {
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::imatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, ab, lda, ldb, stride, batch_size));
}
...
void omatadd_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,
                   ...
                   sycl::buffer<float, 1> &c, int64_t ldc, int64_t stride_c, int64_t batch_size) {
    RETHROW_ONEMKL_EXCEPTIONS(blas_major::omatadd_batch(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, alpha, a, lda, stride_a, beta, b, ldb,
...
sycl::event copy_batch(sycl::queue &queue, int64_t n, const float *x, int64_t incx,
    ...
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::copy_batch(queue, n, x, incx, stridex, y, incy, stridey, batch_size,
                                  dependencies));
}
...
sycl::event copy_batch(sycl::queue &queue, int64_t *n, const float **x, int64_t *incx, float **y,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::copy_batch(queue, n, x, incx, y, incy, group_count, group_size,
                                  dependencies));
}
...
sycl::event axpy_batch(sycl::queue &queue, int64_t n, float alpha, const float *x, int64_t incx,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::axpy_batch(queue, n, alpha, x, incx, stridex, y, incy, stridey, batch_size,
                                  dependencies));
}
...
sycl::event axpy_batch(sycl::queue &queue, int64_t *n, float *alpha, const float **x, int64_t *incx,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::axpy_batch(queue, n, alpha, x, incx, y, incy, group_count, group_size,
                                  dependencies));
}
...
sycl::event gemv_batch(sycl::queue &queue, transpose transa, int64_t m, int64_t n, float alpha,
                       ...
                       int64_t batch_size, const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemv_batch(queue, detail::get_onemkl_transpose(transa), m, n, alpha, a, lda, stride_a, x, incx, stride_x,
...
sycl::event gemv_batch(sycl::queue &queue, transpose *transa, int64_t *m, int64_t *n, float *alpha,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemv_batch(queue, detail::get_onemkl_transpose(transa), m, n, alpha, a, lda, x, incx, beta, y, incy,
...
sycl::event dgmm_batch(sycl::queue &queue, side left_right, int64_t m, int64_t n, const float *a,
                       ...
                       int64_t batch_size, const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::dgmm_batch(queue, detail::get_onemkl_side(left_right), m, n, a, lda, stride_a, x, incx, stride_x, c,
...
sycl::event dgmm_batch(sycl::queue &queue, side *left_right, int64_t *m, int64_t *n,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::dgmm_batch(queue, detail::get_onemkl_side(left_right), m, n, a, lda, x, incx, c, ldc, group_count,
...
sycl::event gemm_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m, int64_t n,
                       ...
                       int64_t ldc, int64_t stride_c, int64_t batch_size,
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemm_batch(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, k, alpha, a, lda, stride_a, b, ldb,
...
sycl::event gemm_batch(sycl::queue &queue, transpose *transa, transpose *transb, int64_t *m,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::gemm_batch(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, k, alpha, a, lda, b, ldb, beta, c,
...
sycl::event trsm_batch(sycl::queue &queue, side left_right, uplo upper_lower, transpose trans,
                       ...
                       int64_t batch_size, const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::trsm_batch(queue, detail::get_onemkl_side(left_right), detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(trans), detail::get_onemkl_diag(unit_diag), m, n, alpha, a,
...
sycl::event syrk_batch(sycl::queue &queue, uplo upper_lower, transpose trans, int64_t n, int64_t k,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::syrk_batch(queue, detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(trans), n, k, alpha, a, lda, stride_a, beta, c,
                                  ldc, stride_c, batch_size, dependencies));
}
...
sycl::event syrk_batch(sycl::queue &queue, uplo *upper_lower, transpose *trans, int64_t *n,
                       ...
                       const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::syrk_batch(queue, detail::get_onemkl_uplo(upper_lower), detail::get_onemkl_transpose(trans), n, k, alpha, a, lda, beta, c, ldc,
                                  group_count, groupsize, dependencies));
}
...
sycl::event omatcopy_batch(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
                           ...
                           int64_t stride_b, int64_t batch_size,
                           const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::omatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, a, lda, stride_a, b, ldb, stride_b,
...
sycl::event imatcopy_batch(sycl::queue &queue, transpose trans, int64_t m, int64_t n, float alpha,
                           ...
                           const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::imatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, ab, lda, ldb, stride, batch_size,
                                      dependencies));
}
...
sycl::event omatadd_batch(sycl::queue &queue, transpose transa, transpose transb, int64_t m,
                          ...
                          int64_t ldc, int64_t stride_c, int64_t batch_size,
                          const std::vector<sycl::event> &dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::omatadd_batch(queue, detail::get_onemkl_transpose(transa), detail::get_onemkl_transpose(transb), m, n, alpha, a, lda, stride_a, beta, b,
...
sycl::event omatcopy_batch(sycl::queue& queue, transpose* trans, int64_t* m, int64_t* n,
                           ...
                           int64_t group_count, int64_t* groupsize,
                           const std::vector<sycl::event>& dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::omatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, a, lda, b, ldb, group_count,
                                      groupsize, dependencies));
}
...
sycl::event imatcopy_batch(sycl::queue& queue, transpose* trans, int64_t* m, int64_t* n,
                           ...
                           int64_t group_count, int64_t* groupsize,
                           const std::vector<sycl::event>& dependencies) {
    RETHROW_ONEMKL_EXCEPTIONS_RET(blas_major::imatcopy_batch(queue, detail::get_onemkl_transpose(trans), m, n, alpha, ab, lda, ldb, group_count,
                                      groupsize, dependencies));
}
