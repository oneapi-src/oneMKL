...
void copy_batch(cl::sycl::queue &queue, int64_t n, cl::sycl::buffer<float, 1> &x, int64_t incx,
                std::int64_t stridex, cl::sycl::buffer<float, 1> &y, int64_t incy,
...
void copy_batch(cl::sycl::queue &queue, int64_t n, cl::sycl::buffer<double, 1> &x, int64_t incx,
                std::int64_t stridex, cl::sycl::buffer<double, 1> &y, int64_t incy,
                std::int64_t stridey, std::int64_t batch_size) {
...
void copy_batch(cl::sycl::queue &queue, int64_t n, cl::sycl::buffer<std::complex<float>, 1> &x,
                int64_t incx, std::int64_t stridex, cl::sycl::buffer<std::complex<float>, 1> &y,
...
void copy_batch(cl::sycl::queue &queue, int64_t n, cl::sycl::buffer<std::complex<double>, 1> &x,
                int64_t incx, std::int64_t stridex, cl::sycl::buffer<std::complex<double>, 1> &y,
                int64_t incy, std::int64_t stridey, std::int64_t batch_size) {
...
cl::sycl::event copy_batch(cl::sycl::queue &queue, int64_t *n, const float **x, int64_t *incx,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *copy_batch_event = new cl::sycl::event(
...
cl::sycl::event gemv_batch(cl::sycl::queue &queue, transpose *transa, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *gemv_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::sgemv_batch_sycl(
...
cl::sycl::event gemv_batch(cl::sycl::queue &queue, transpose *transa, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *gemv_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::dgemv_batch_sycl(
                &queue, MAJOR, mkl_convert(transa[i]), m[i], n[i], alpha[i], a, lda[i], x, incx[i],
                beta[i], y, incy[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(gemv_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event gemv_batch(cl::sycl::queue &queue, transpose *transa, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *gemv_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::cgemv_batch_sycl(
                &queue, MAJOR, mkl_convert(transa[i]), m[i], n[i], alpha[i], a, lda[i], x, incx[i],
                beta[i], y, incy[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(gemv_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event gemv_batch(cl::sycl::queue &queue, transpose *transa, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *gemv_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::zgemv_batch_sycl(
                &queue, MAJOR, mkl_convert(transa[i]), m[i], n[i], alpha[i], a, lda[i], x, incx[i],
                beta[i], y, incy[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(gemv_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event dgmm_batch(cl::sycl::queue &queue, side *left_right, int64_t *m, int64_t *n,
    ...
    int64_t total_groupsize = 0;
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *dgmm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::sdgmm_batch_sycl(
...
cl::sycl::event dgmm_batch(cl::sycl::queue &queue, side *left_right, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *dgmm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::ddgmm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), m[i], n[i], a, lda[i], x, incx[i], c,
                ldc[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(dgmm_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event dgmm_batch(cl::sycl::queue &queue, side *left_right, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *dgmm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::cdgmm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), m[i], n[i], a, lda[i], x, incx[i], c,
                ldc[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(dgmm_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event dgmm_batch(cl::sycl::queue &queue, side *left_right, int64_t *m, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *dgmm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::zdgmm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), m[i], n[i], a, lda[i], x, incx[i], c,
                ldc[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(dgmm_batch_event);
        total_groupsize += groupsize[i];
...
cl::sycl::event gemm_batch(cl::sycl::queue &queue, transpose *transa, transpose *transb, int64_t *m,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *gemm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::hgemm_batch_sycl(
                &queue, MAJOR, mkl_convert(transa[i]), mkl_convert(transb[i]), m[i], n[i], k[i],
...
cl::sycl::event trsm_batch(cl::sycl::queue &queue, side *left_right, uplo *upper_lower,
    ...
    int64_t total_groupsize = 0;
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *trsm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::strsm_batch_sycl(
...
cl::sycl::event trsm_batch(cl::sycl::queue &queue, side *left_right, uplo *upper_lower,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *trsm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::dtrsm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), mkl_convert(upper_lower[i]),
                mkl_convert(trans[i]), mkl_convert(unit_diag[i]), m[i], n[i], alpha[i], a, lda[i],
                b, ldb[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(trsm_batch_event);
...
cl::sycl::event trsm_batch(cl::sycl::queue &queue, side *left_right, uplo *upper_lower,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *trsm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::ctrsm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), mkl_convert(upper_lower[i]),
                mkl_convert(trans[i]), mkl_convert(unit_diag[i]), m[i], n[i], alpha[i], a, lda[i],
                b, ldb[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(trsm_batch_event);
...
cl::sycl::event trsm_batch(cl::sycl::queue &queue, side *left_right, uplo *upper_lower,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *trsm_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::ztrsm_batch_sycl(
                &queue, MAJOR, mkl_convert(left_right[i]), mkl_convert(upper_lower[i]),
                mkl_convert(trans[i]), mkl_convert(unit_diag[i]), m[i], n[i], alpha[i], a, lda[i],
                b, ldb[i], total_groupsize, groupsize[i], dependencies));
        coalesced_events.push_back(trsm_batch_event);
...
cl::sycl::event syrk_batch(cl::sycl::queue &queue, uplo *upper_lower, transpose *trans, int64_t *n,
    ...
    int64_t total_groupsize = 0;
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *syrk_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::ssyrk_batch_sycl(
...
cl::sycl::event syrk_batch(cl::sycl::queue &queue, uplo *upper_lower, transpose *trans, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *syrk_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::dsyrk_batch_sycl(
                &queue, MAJOR, mkl_convert(upper_lower[i]), mkl_convert(trans[i]), n[i], k[i],
                alpha[i], a, lda[i], beta[i], c, ldc[i], total_groupsize, groupsize[i],
                dependencies));
        coalesced_events.push_back(syrk_batch_event);
...
cl::sycl::event syrk_batch(cl::sycl::queue &queue, uplo *upper_lower, transpose *trans, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *syrk_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::csyrk_batch_sycl(
                &queue, MAJOR, mkl_convert(upper_lower[i]), mkl_convert(trans[i]), n[i], k[i],
                alpha[i], a, lda[i], beta[i], c, ldc[i], total_groupsize, groupsize[i],
                dependencies));
        coalesced_events.push_back(syrk_batch_event);
...
cl::sycl::event syrk_batch(cl::sycl::queue &queue, uplo *upper_lower, transpose *trans, int64_t *n,
    ...
    for (int64_t i = 0; i < group_count; i++) {
        cl::sycl::event *syrk_batch_event =
            new cl::sycl::event(::oneapi::mkl::gpu::zsyrk_batch_sycl(
                &queue, MAJOR, mkl_convert(upper_lower[i]), mkl_convert(trans[i]), n[i], k[i],
                alpha[i], a, lda[i], beta[i], c, ldc[i], total_groupsize, groupsize[i],
                dependencies));
        coalesced_events.push_back(syrk_batch_event);
