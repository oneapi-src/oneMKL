...
inline void gemm(Func func, DATATYPE_A DT_A, DATATYPE_B DT_B, DATATYPE_C DT_C, sycl::queue &queue,
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            rocblas_status err;
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_operation(transa),
                               get_rocblas_operation(transb), m, n, k, (rocDataType_C *)&alpha, a_,
                               DT_A, lda, b_, DT_B, ldb, (rocDataType_C *)&beta, c_, DT_C, ldc,
                               DT_C, rocblas_gemm_algo_standard);
        });
    });
}
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_side_mode(left_right),
                               get_rocblas_fill_mode(upper_lower), m, n, (rocDataType *)&alpha, a_,
                               lda, b_, ldb, (rocDataType *)&beta, c_, ldc);
        });
    });
}
...
inline void herk(Func func, sycl::queue &queue, uplo upper_lower, transpose trans, int64_t n,
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            rocblas_status err;
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_fill_mode(upper_lower),
                               get_rocblas_operation(trans), n, k, (rocScalarType *)&alpha, a_, lda,
                               (rocScalarType *)&beta, c_, ldc);
        });
    });
}
...
inline void syr2k(Func func, sycl::queue &queue, uplo upper_lower, transpose trans, int64_t n,
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_fill_mode(upper_lower),
                               get_rocblas_operation(trans), n, k, (rocDataType *)&alpha, a_, lda,
                               b_, ldb, (rocDataType *)&beta, c_, ldc);
        });
    });
}
...
inline void her2k(Func func, sycl::queue &queue, uplo upper_lower, transpose trans, int64_t n,
    ...
    queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_fill_mode(upper_lower),
                               get_rocblas_operation(trans), n, k, (rocDataType *)&alpha, a_, lda,
                               b_, ldb, (rocScalarType *)&beta, c_, ldc);
        });
    });
}
...
inline sycl::event gemm(Func func, DATATYPE_A DT_A, DATATYPE_B DT_B, DATATYPE_C DT_C,
    ...
    auto done = queue.submit([&](sycl::handler &cgh) {
        ...
        onemkl_rocblas_host_task(cgh, queue, [=](RocblasScopedContextHandler &sc) {
            ...
            ROCBLAS_ERROR_FUNC(func, err, handle, get_rocblas_operation(transa),
                               get_rocblas_operation(transb), m, n, k, (rocDataType_C *)&alpha, a_,
                               DT_A, lda, b_, DT_B, ldb, (rocDataType_C *)&beta, c_, DT_C, ldc, c_,
                               DT_C, ldc, DT_C, rocblas_gemm_algo_standard, 0, 0);
        });
    });
    return done;
}
