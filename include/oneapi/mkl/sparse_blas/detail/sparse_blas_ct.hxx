...
std::enable_if_t<detail::are_fp_int_supported_v<fpType, intType>> set_csr_data(
    ...
    intType num_cols, index_base index, sycl::buffer<intType, 1> &row_ptr,
    sycl::buffer<intType, 1> &col_ind, sycl::buffer<fpType, 1> &val) {
    BACKEND::set_csr_data(selector.get_queue(), handle, num_rows, num_cols, index, row_ptr, col_ind,
                          val);
}
...
std::enable_if_t<detail::are_fp_int_supported_v<fpType, intType>, sycl::event> set_csr_data(
    backend_selector<backend::BACKEND> selector, matrix_handle_t handle, intType num_rows,
    intType num_cols, index_base index, intType *row_ptr, intType *col_ind, fpType *val,
    const std::vector<sycl::event> &dependencies = {}) {
    return BACKEND::set_csr_data(selector.get_queue(), handle, num_rows, num_cols, index, row_ptr,
                                 col_ind, val, dependencies);
}
