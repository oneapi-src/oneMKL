...
void axpy_batch(backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<float> alpha,
...
void axpy_batch(backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<double> alpha,
...
void gemv_batch(backend_selector<backend::mklgpu> selector, transpose trans, std::int64_t m, std::int64_t n,
...
void dgmm_batch(backend_selector<backend::mklgpu> selector, side left_right,
    ...
    oneapi::mkl::blas::mklgpu::MAJOR::dgmm_batch(selector.get_queue(), left_right, m, n, a, lda, stridea, x, incx, stridex, c, ldc, stridec, batch_size);
...
cl::sycl::event axpy_batch(backend_selector<backend::mklgpu> selector, std::int64_t n, float alpha,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::axpy_batch(selector.get_queue(), n, alpha, x, incx, stridex, y, incy, stridey, batch_size, dependencies);
...
cl::sycl::event axpy_batch(backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<float> alpha,
...
cl::sycl::event axpy_batch(backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<double> alpha,
...
cl::sycl::event axpby(
    backend_selector<backend::mklgpu> selector, std::int64_t n, float alpha, const float *x, std::int64_t incx, const float beta, 
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::axpby(selector.get_queue(), n, alpha, x, incx, beta, y, incy, dependencies);
    axpby_postcondition(selector.get_queue(), n, alpha, x, incx, beta, y, incy, dependencies);
    return done;
}

cl::sycl::event axpby(
    backend_selector<backend::mklgpu> selector, std::int64_t n, double alpha, const double *x, std::int64_t incx, const double beta, 
...
cl::sycl::event axpby(
    backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<float> alpha, const std::complex<float> *x, std::int64_t incx, const std::complex<float> beta, 
...
cl::sycl::event axpby(
    backend_selector<backend::mklgpu> selector, std::int64_t n, std::complex<double> alpha, const std::complex<double> *x, std::int64_t incx, const std::complex<double> beta, 
...
cl::sycl::event gemv_batch(oneapi::mkl::device libkey, cl::sycl::queue &queue, transpose trans,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::gemv_batch(selector.get_queue(), trans, m, n, alpha, a, lda, stridea, x, incx, stridex, beta, y, incy, stridey, batch_size, dependencies);
...
cl::sycl::event gemv_batch(oneapi::mkl::device libkey, cl::sycl::queue &queue, transpose *trans,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::gemv_batch(selector.get_queue(), trans, m, n, alpha, a, lda, x, incx, beta, y, incy, group_count, group_size, dependencies);
...
cl::sycl::event dgmm_batch(backend_selector<backend::mklgpu> selector, side left_right,
              std::int64_t m, std::int64_t n,
              const float *a, std::int64_t lda, std::int64_t stridea,
              const float *x, std::int64_t incx, std::int64_t stridex,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::dgmm_batch(selector.get_queue(), left_right, m, n, a, lda, stridea, x, incx, stridex, c, ldc, stridec, batch_size, dependencies);
...
cl::sycl::event dgmm_batch(backend_selector<backend::mklgpu> selector, side *left_right,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::dgmm_batch(selector.get_queue(), left_right, m, n, a, lda, x, incx, c, ldc, group_count, group_size, dependencies);
...
cl::sycl::event gemm_bias(backend_selector<backend::mklgpu> selector, transpose transa, transpose transb,
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::gemm_bias(selector.get_queue(), transa, transb, offsetc, m, n, k, alpha, a, lda, ao, b, ldb, bo, beta, c, ldc, co, dependencies);
...
cl::sycl::event trsm_batch(backend_selector<backend::mklgpu> selector, side left_right, uplo upper_lower, 
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::trsm_batch(selector.get_queue(), left_right, upper_lower, trans, unit_diag, m, n, alpha, a, lda, stride_a, b, ldb, stride_b, batch_size, dependencies);
...
cl::sycl::event trsm_batch(backend_selector<backend::mklgpu> selector, side *left_right, uplo *upper_lower, 
    ...
    auto done = oneapi::mkl::blas::mklgpu::MAJOR::trsm_batch(selector.get_queue(), left_right, upper_lower, trans, unit_diag, m, n, alpha, a, lda, b, ldb, group_count, group_size, dependencies);
