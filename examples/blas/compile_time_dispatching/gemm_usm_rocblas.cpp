...
*       Matrix-Matrix Multiplication on a SYCL device with ROCm backend.
*
...
void run_gemm_example(const cl::sycl::device &dev) {
    ...
    try {
        printf("CUBLAS\n");
        gemm_done = oneapi::mkl::blas::column_major::gemm(oneapi::mkl::backend_selector<oneapi::mkl::backend::rocblas> {main_queue}, transA, transB, m, n, k, alpha, A, ldA, B, ldB, beta,  C, ldC);
    }
    catch(cl::sycl::exception const& e) {
