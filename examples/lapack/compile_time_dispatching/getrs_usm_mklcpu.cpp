...
*       to perform LU factorization and compute the solution on an Intel CPU SYCL device.
*
...
void run_getrs_example(const sycl::device& device)
{
    ...
    try {
        ...
        std::int64_t getrf_scratchpad_size = oneapi::mkl::lapack::getrf_scratchpad_size<float>(oneapi::mkl::backend_selector<oneapi::mkl::backend::mklcpu> {queue}, n, n, lda);
        std::int64_t getrs_scratchpad_size = oneapi::mkl::lapack::getrs_scratchpad_size<float>(oneapi::mkl::backend_selector<oneapi::mkl::backend::mklcpu> {queue}, oneapi::mkl::transpose::nontrans, n, nhs, lda, ldb);
        ...
        sycl::event getrf_done_event = oneapi::mkl::lapack::getrf(oneapi::mkl::backend_selector<oneapi::mkl::backend::mklcpu> {queue}, n, n, A, lda, ipiv, getrf_scratchpad, getrf_scratchpad_size);
        sycl::event getrs_done_event = oneapi::mkl::lapack::getrs(oneapi::mkl::backend_selector<oneapi::mkl::backend::mklcpu> {queue}, oneapi::mkl::transpose::nontrans, n, nhs, A, lda, ipiv, B, ldb, getrs_scratchpad, getrs_scratchpad_size, {getrf_done_event});
