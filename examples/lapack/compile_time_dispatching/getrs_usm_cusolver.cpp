...
*       to perform LU factorization and compute the solution on a NVIDIA GPU SYCL device.
*
...
void run_getrs_example(const sycl::device& device)
{
    ...
    try {
        // USM
        float *A = (float *) malloc_shared(A_size * sizeof(float), device, context);
        float *B = (float *) malloc_shared(B_size * sizeof(float), device, context);
        std::int64_t *ipiv = (std::int64_t*) malloc_shared(ipiv_size * sizeof(std::int64_t), device, context);

        // Get sizes of scratchpads for calculations
        std::int64_t getrf_scratchpad_size = oneapi::mkl::lapack::getrf_scratchpad_size<float>(oneapi::mkl::backend_selector<oneapi::mkl::backend::cusolver> {queue}, n, n, lda);
        std::int64_t getrs_scratchpad_size = oneapi::mkl::lapack::getrs_scratchpad_size<float>(oneapi::mkl::backend_selector<oneapi::mkl::backend::cusolver> {queue}, oneapi::mkl::transpose::nontrans, n, nrhs, lda, ldb);

        float *getrf_scratchpad = (float*) malloc_shared(getrf_scratchpad_size * sizeof(float), device, context);
        float *getrs_scratchpad = (float*) malloc_shared(getrs_scratchpad_size * sizeof(float), device, context);

        // Skip checking getrs scratchpad memory allocation on cusolver because it does not use scrachpad memory
        ...
        sycl::event getrf_done_event = oneapi::mkl::lapack::getrf(oneapi::mkl::backend_selector<oneapi::mkl::backend::cusolver> {queue}, n, n, A, lda, ipiv, getrf_scratchpad, getrf_scratchpad_size);
        sycl::event getrs_done_event = oneapi::mkl::lapack::getrs(oneapi::mkl::backend_selector<oneapi::mkl::backend::cusolver> {queue}, oneapi::mkl::transpose::nontrans, n, nrhs, A, lda, ipiv, B, ldb, getrs_scratchpad, getrs_scratchpad_size, {getrf_done_event});
