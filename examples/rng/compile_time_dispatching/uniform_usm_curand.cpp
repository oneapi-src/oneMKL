...
*       a NVIDIA GPU SYCL device with Unified Shared Memory(USM) API.
*
...
bool run_uniform_example(const sycl::device &dev) {
    ...
    oneapi::mkl::rng::default_engine engine(oneapi::mkl::backend_selector<oneapi::mkl::backend::curand> {queue}, seed);
...
int main (int argc, char ** argv) {
    ...
    if (my_dev.is_gpu() && vendor_id == NVIDIA_ID) {
        std::cout << "Running tests on GPU device. Device name is: " << my_dev.get_info<sycl::info::device::name>() << ".\n";
    } else {
