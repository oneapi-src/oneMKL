...
std::pair<std::int64_t,std::int64_t> get_default_distances(const std::vector<std::int64_t>& sizes, const std::vector<std::int64_t>& strides_fwd, const std::vector<std::int64_t>& strides_bwd){
        ...
        for(std::int64_t i=0;i<size0;i++){
            for(std::int64_t j=0;j<size1;j++){
                for(std::int64_t k=0;k<size2;k++){
                    res[cast_unsigned(b * distance + i * stride1 + j * stride2 + k * stride3 + stride0)] = contiguous[cast_unsigned(((b * size0 + i) * size1 + j) * size2 + k)];
                }
            }
        }
    }
...
struct DFTParams {
    ...
    std::int64_t batches;
    DFTParams(std::vector<std::int64_t> sizes, std::int64_t batches) : sizes(sizes), strides_fwd({}), strides_bwd({}), batches(batches) {}
    DFTParams(std::vector<std::int64_t> sizes, std::vector<std::int64_t> strides_fwd, std::vector<std::int64_t> strides_bwd, std::int64_t batches) : sizes(sizes), strides_fwd(strides_fwd), strides_bwd(strides_bwd), batches(batches) {}
};
...
public:
    std::string operator()(
        ...
        if(params.strides_fwd.size() != 0){
            ...
            std::for_each(params.strides_fwd.begin(), params.strides_fwd.end() - 1,
        ...
        if(params.strides_bwd.size() != 0){
            ...
            // intersperse strides with "_"
            std::for_each(params.strides_bwd.begin(), params.strides_bwd.end() - 1,
                        [&info_name](auto s) { info_name.append(std::to_string(s)).append("_"); });
