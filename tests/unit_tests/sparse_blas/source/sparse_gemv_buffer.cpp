...
int test(sycl::device *dev, intType nrows, intType ncols, double density_A_matrix,
    ...
    generate_random_matrix<fpType, intType>(nrows, ncols, density_A_matrix, int_index, ia_host,
                                            ja_host, a_host);
    ...
    try {
        ...
        CALL_RT_OR_CT(oneapi::mkl::sparse::set_csr_data, main_queue, handle, nrows, ncols, index,
                      ia_buf, ja_buf, a_buf);
    ...
    prepare_reference_gemv_data(ia_host.data(), ja_host.data(), a_host.data(), nrows, ncols,
                                a_host.size(), int_index, transpose_val, alpha, beta, x_host.data(),
                                y_ref_host.data());
