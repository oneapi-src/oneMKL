...
bool accuracy(const sycl::device &dev, oneapi::mkl::transpose trans, int64_t n, int64_t nrhs, int64_t lda, int64_t stride_a, int64_t stride_ipiv, int64_t ldb, int64_t stride_b, int64_t batch_size, uint64_t seed) {
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::getrs_batch(queue, trans, n, nrhs, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, B_dev, ldb, stride_b, batch_size, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::getrs_batch, trans, n, nrhs, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, B_dev, ldb, stride_b, batch_size, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    bool result = true;
    for (int64_t i = 0; i < batch_size; i++)
       if(!check_getrs_accuracy(trans, n, nrhs, A.data()+i*stride_a, lda, ipiv.data()+i*stride_ipiv, B.data()+i*stride_b, ldb, A_initial.data()+i*stride_a, B_initial.data()+i*stride_b) ) {
...
INSTANTIATE_TEST_SUITE_P(GetrsBatchStrideTestSuite, GetrsBatchStrideTests, ::testing::ValuesIn(devices), DeviceNamePrint());
