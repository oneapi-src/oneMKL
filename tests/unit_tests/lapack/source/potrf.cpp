...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::potrf_scratchpad_size<fp>(queue, uplo, n, lda);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::potrf_scratchpad_size<fp>, uplo, n, lda);
#endif
...
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::potrf, uplo, n, A_dev, lda, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::potrf(queue, uplo, n, A_dev, lda, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::potrf, uplo, n, A_dev, lda, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
INSTANTIATE_TEST_SUITE_P(PotrfTestSuite, PotrfTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE(Potrf)
#endif
