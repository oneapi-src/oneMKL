...
bool accuracy(const sycl::device &dev, uint64_t seed) {
    ...
    /* Test Parameters */
    std::vector<oneapi::mkl::uplo> uplo_vec = {oneapi::mkl::uplo::upper, oneapi::mkl::uplo::lower};
    ...
    /* Compute on device */
    {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::potrs_batch_scratchpad_size<fp>(queue, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), lda_vec.data(), ldb_vec.data(), group_count, group_sizes_vec.data());
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::potrs_batch_scratchpad_size<fp>, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), lda_vec.data(), ldb_vec.data(), group_count, group_sizes_vec.data());
#endif
        ...
        for ( int64_t global_id = 0; global_id < batch_size; global_id++, A_dev_iter++, B_dev_iter++ ) {
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::potrs_batch(queue, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::potrs_batch, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    for( int64_t group_id = 0; group_id < group_count; group_id++) {
        ...
        auto ldb = ldb_vec[group_id];
        auto group_size = group_sizes_vec[group_id];
        for ( int64_t local_id = 0; local_id < group_size; local_id++, global_id++, A_iter++, B_iter++, A_initial_iter++, B_initial_iter++ ) {
            if( !check_potrs_accuracy(uplo, n, nrhs, A_iter->data(), lda, B_iter->data(), ldb, A_initial_iter->data(), B_initial_iter->data())  ) {
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::potrs_batch(queue, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::potrs_batch, uplo_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
INSTANTIATE_TEST_SUITE_P(PotrsBatchGroupTestSuite, PotrsBatchGroupTests, ::testing::ValuesIn(devices), DeviceNamePrint());
