...
bool accuracy(const sycl::device &dev, uint64_t seed) {
    ...
    /* Compute on device */
    {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::getri_batch_scratchpad_size<fp>(queue, n_vec.data(), lda_vec.data(), group_count, group_sizes_vec.data());
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::getri_batch_scratchpad_size<fp>, n_vec.data(), lda_vec.data(), group_count, group_sizes_vec.data());
#endif
        ...
        for ( int64_t global_id = 0; global_id < batch_size; global_id++, A_iter++, ipiv_iter++ ) {
            ...
            host_to_device_copy(queue, ipiv_iter->data(), ipiv_dev_ptrs[global_id], ipiv_iter->size());
        }
        queue.wait_and_throw();

#ifdef CALL_RT_API
        oneapi::mkl::lapack::getri_batch(queue, n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::getri_batch, n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    for( int64_t group_id = 0; group_id < group_count; group_id++) {
        ...
        for ( int64_t local_id = 0; local_id < group_size; local_id++, global_id++, A_iter++, ipiv_iter++, A_initial_iter++ ) {
            if( !check_getri_accuracy(n, A_iter->data(), lda, ipiv_iter->data(), A_initial_iter->data()) ) {
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::getri_batch(queue, n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::getri_batch, n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
INSTANTIATE_TEST_SUITE_P(GetriBatchGroupTestSuite, GetriBatchGroupTests, ::testing::ValuesIn(devices), DeviceNamePrint());
