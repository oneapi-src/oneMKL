...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::getrf_scratchpad_size<fp>(queue, m, n, lda);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::getrf_scratchpad_size<fp>, m, n, lda);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::getrf(queue, m, n, A_dev, lda, ipiv_dev, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::getrf, m, n, A_dev, lda, ipiv_dev, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::getrf(queue, m, n, A_dev, lda, ipiv_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::getrf, m, n, A_dev, lda, ipiv_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
INSTANTIATE_TEST_SUITE_P(GetrfTestSuite, GetrfTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE(Getrf)
#endif
