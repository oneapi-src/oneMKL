...
bool accuracy(const sycl::device &dev, int64_t n, int64_t lda, int64_t stride_a, int64_t stride_ipiv, int64_t batch_size, uint64_t seed) {
    ...
    for (int64_t i = 0; i < batch_size; i++) {
        auto info = reference::getrf(n, n, A.data()+i*stride_a, lda, ipiv.data()+i*stride_ipiv);
        if( 0 != info) {
            global::log << "\tbatch routine index " << i << ": reference getrf failed with info: " << info << std::endl;
            return false;
        }
    }
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::getri_batch_scratchpad_size<fp>(queue, n, lda, stride_a, stride_ipiv, batch_size);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::getri_batch_scratchpad_size<fp>, n, lda, stride_a, stride_ipiv, batch_size);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::getri_batch(queue, n, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, batch_size, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::getri_batch, n, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, batch_size, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    bool result = true;
    for (int64_t i = 0; i < batch_size; i++)
       if(!check_getri_accuracy(n, A.data()+i*stride_a, lda, ipiv.data()+i*stride_ipiv, A_initial.data()+i*stride_a) ) {
...
template <typename mem_T>
bool usm_dependency(const sycl::device &dev, int64_t n, int64_t lda, int64_t stride_a, int64_t stride_ipiv, int64_t batch_size, uint64_t seed) {
    using fp = typename mem_T_info<mem_T>::value_type;
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::getri_batch(queue, n, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, batch_size, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::getri_batch, n, A_dev, lda, stride_a, ipiv_dev, stride_ipiv, batch_size, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
INSTANTIATE_TEST_SUITE_P(GetriBatchStrideTestSuite, GetriBatchStrideTests, ::testing::ValuesIn(devices), DeviceNamePrint());
