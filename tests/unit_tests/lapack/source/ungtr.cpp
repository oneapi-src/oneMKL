...
bool accuracy(const sycl::device &dev, oneapi::mkl::uplo uplo, int64_t n, int64_t lda, uint64_t seed) {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::ungtr_scratchpad_size<fp>(queue, uplo, n, lda);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::ungtr_scratchpad_size<fp>, uplo, n, lda);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::ungtr(queue, uplo, n, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::ungtr, uplo, n, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    return check_or_un_gtr_accuracy(n, A.data(), lda);
}
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::ungtr(queue, uplo, n, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::ungtr, uplo, n, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
class UngtrTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(UngtrTestSuite, UngtrTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE_COMPLEX(Ungtr)
#endif
