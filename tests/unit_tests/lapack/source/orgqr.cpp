...
bool accuracy( const sycl::device& dev, int64_t m, int64_t n, int64_t k, int64_t lda, uint64_t seed) {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::orgqr_scratchpad_size<fp>(queue, m, n, k, lda);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::orgqr_scratchpad_size<fp>, m, n, k, lda);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::orgqr(queue, m, n, k, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::orgqr, m, n, k, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
...
template <typename mem_T>
bool usm_dependency( const sycl::device& dev, int64_t m, int64_t n, int64_t k, int64_t lda, uint64_t seed) {
    using fp = typename mem_T_info<mem_T>::value_type;
    using fp_real = typename complex_info<fp>::real_type;
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::orgqr(queue, m, n, k, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::orgqr, m, n, k, A_dev, lda, tau_dev, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
class OrgqrTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(OrgqrTestSuite, OrgqrTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE_REAL(Orgqr)
#endif
