...
bool accuracy(const sycl::device& dev, oneapi::mkl::side left_right, oneapi::mkl::transpose trans, int64_t m, int64_t n, int64_t k, int64_t lda, int64_t ldc, uint64_t seed) {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::ormqr_scratchpad_size<fp>(queue, left_right, trans, m, n, k, lda, ldc);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::ormqr_scratchpad_size<fp>, left_right, trans, m, n, k, lda, ldc);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::ormqr(queue, left_right, trans, m, n, k, A_dev, lda, tau_dev, C_dev, ldc, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::ormqr, left_right, trans, m, n, k, A_dev, lda, tau_dev, C_dev, ldc, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    auto ldqc = ldc;
    info = reference::or_un_mqr(left_right, trans, m, n, k, A.data(), lda, tau.data(), QC_ref.data(), ldqc);
    if ( 0 != info) {
        global::log << "\treference ormqr failed with info: " << info << std::endl;
        return false;
    }
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::ormqr(queue, left_right, trans, m, n, k, A_dev, lda, tau_dev, C_dev, ldc, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::ormqr, left_right, trans, m, n, k, A_dev, lda, tau_dev, C_dev, ldc, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
class OrmqrTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(OrmqrTestSuite, OrmqrTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE_REAL(Ormqr)
#endif
