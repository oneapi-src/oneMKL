...
bool accuracy(const sycl::device &dev, int64_t m, int64_t n, int64_t lda, int64_t stride_a, int64_t stride_tau, int64_t batch_size, uint64_t seed) {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::geqrf_batch_scratchpad_size<fp>(queue, m, n, lda, stride_a, stride_tau, batch_size);
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::geqrf_batch_scratchpad_size<fp>, m, n, lda, stride_a, stride_tau, batch_size);
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::geqrf_batch(queue, m, n, A_dev, lda, stride_a, tau_dev, stride_tau, batch_size, scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::geqrf_batch, m, n, A_dev, lda, stride_a, tau_dev, stride_tau, batch_size, scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    bool result = true;
    for (int64_t i = 0; i < batch_size; i++)
       if(!check_geqrf_accuracy(A.data()+i*stride_a, A_initial.data()+i*stride_a, tau.data()+i*stride_tau, m, n, lda) ) {
...
template <typename mem_T>
bool usm_dependency(const sycl::device &dev, int64_t m, int64_t n, int64_t lda, int64_t stride_a, int64_t stride_tau, int64_t batch_size, uint64_t seed) {
    using fp = typename mem_T_info<mem_T>::value_type;
    using fp_real = typename complex_info<fp>::real_type;
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::geqrf_batch(queue, m, n, A_dev, lda, stride_a, tau_dev, stride_tau, batch_size, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::geqrf_batch, m, n, A_dev, lda, stride_a, tau_dev, stride_tau, batch_size, scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
class GeqrfBatchStrideTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(GeqrfBatchStrideTestSuite, GeqrfBatchStrideTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE(GeqrfBatchStride)
#endif
