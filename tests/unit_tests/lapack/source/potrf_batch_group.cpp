...
bool accuracy(const sycl::device &dev, uint64_t seed) {
...
#ifdef CALL_RT_API
        const auto scratchpad_size = oneapi::mkl::lapack::potrf_batch_scratchpad_size<fp>(queue, uplo_vec.data(), n_vec.data(), lda_vec.data(), group_count, group_sizes_vec.data());
#else
        int64_t scratchpad_size;
        TEST_RUN_CT_SELECT(queue, scratchpad_size = oneapi::mkl::lapack::potrf_batch_scratchpad_size<fp>, uplo_vec.data(), n_vec.data(), lda_vec.data(), group_count, group_sizes_vec.data());
#endif
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::potrf_batch(queue, uplo_vec.data(), n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::potrf_batch, uplo_vec.data(), n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    for( int64_t group_id = 0; group_id < group_count; group_id++) {
        ...
        auto lda = lda_vec[group_id];
        auto group_size = group_sizes_vec[group_id];
        for ( int64_t local_id = 0; local_id < group_size; local_id++, global_id++, A_iter++, A_initial_iter++ ) {
            if(!check_potrf_accuracy(A_initial_iter->data(), A_iter->data(), uplo, n, lda ) ) {
...
#ifdef CALL_RT_API
        sycl::event func_event = oneapi::mkl::lapack::potrf_batch(queue, uplo_vec.data(), n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#else
        sycl::event func_event;
        TEST_RUN_CT_SELECT(queue, sycl::event func_event = oneapi::mkl::lapack::potrf_batch, uplo_vec.data(), n_vec.data(), A_dev_ptrs.data(), lda_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size, sycl::vector_class<sycl::event>{in_event});
#endif
...
class PotrfBatchGroupTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(PotrfBatchGroupTestSuite, PotrfBatchGroupTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE_USM(PotrfBatchGroup)
#endif
