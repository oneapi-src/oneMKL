...
bool accuracy(const sycl::device &dev, uint64_t seed) {
    ...
    /* Test Parameters */
    std::vector<oneapi::mkl::transpose> trans_vec = {oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::trans};
    ...
    /* Compute on device */
    {
        ...
        for ( int64_t global_id = 0; global_id < batch_size; global_id++, A_iter++, B_iter++, ipiv_iter++ ) {
        ...
        for ( int64_t global_id = 0; global_id < batch_size; global_id++, A_dev_iter++, B_dev_iter++, ipiv_dev_iter++ ) {
...
#ifdef CALL_RT_API
        oneapi::mkl::lapack::getrs_batch(queue, trans_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#else
        TEST_RUN_CT_SELECT(queue, oneapi::mkl::lapack::getrs_batch, trans_vec.data(), n_vec.data(), nrhs_vec.data(), A_dev_ptrs.data(), lda_vec.data(), ipiv_dev_ptrs.data(), B_dev_ptrs.data(), ldb_vec.data(), group_count, group_sizes_vec.data(), scratchpad_dev, scratchpad_size);
#endif
        queue.wait_and_throw();
    ...
    for( int64_t group_id = 0; group_id < group_count; group_id++) {
        ...
        auto ldb = ldb_vec[group_id];
        auto group_size = group_sizes_vec[group_id];
        for ( int64_t local_id = 0; local_id < group_size; local_id++, global_id++, A_iter++, B_iter++, ipiv_iter++, A_initial_iter++, B_initial_iter++ ) {
            if( !check_getrs_accuracy(trans, n, nrhs, A_iter->data(), lda, ipiv_iter->data(), B_iter->data(), ldb, A_initial_iter->data(), B_initial_iter->data())  ) {
...
class GetrsBatchGroupTests : public ::testing::TestWithParam<sycl::device*> {};
INSTANTIATE_TEST_SUITE_P(GetrsBatchGroupTestSuite, GetrsBatchGroupTests, ::testing::ValuesIn(devices), DeviceNamePrint());
RUN_SUITE_USM(GetrsBatchGroup)
#endif
