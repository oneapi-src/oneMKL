...
bool rel_mat_err_check(int64_t m, int64_t n, const fp* A, int64_t lda, const fp* Ref, int64_t ldr, float threshold = 10.0, char norm_type = '1') {
    ...
    if(!result) {
        snprintf(global::buffer.data(), global::buffer.size(), "\t|A - Ref| / (|Ref| min(m,n) eps) = |%e| / (|%e| %d * %e) = %e\n", norm_residual, norm_Ref, static_cast<int>(std::min(m,n)), ulp, rel_err);
...
template <typename fp>
bool rel_id_err_check(int64_t n, const fp* A, int64_t lda, float threshold = 10.0, char norm_type = '1') {
    using fp_real = typename complex_info<fp>::real_type;
    ...
    bool result = rel_err < threshold;
    if(!result) {
        snprintf(global::buffer.data(), global::buffer.size(), "\t|A - I| / (|I| n eps) = |%e| / (|%d| %d * %e) = %e\n", norm_residual, static_cast<int>(n), static_cast<int>(n), ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
    }
    return result;
...
template <typename fp>
bool rel_vec_err_check(int64_t n, const fp* A, const fp* Ref, float threshold = 10.0, char norm_type = '1') {
    using fp_real = typename complex_info<fp>::real_type;
    ...
    bool result = rel_err < threshold;
    if(!result) {
        snprintf(global::buffer.data(), global::buffer.size(), "\t|V - Ref| / (|Ref| eps) = |%e| / (|%e| %e) = %e\n", norm_residual, norm_Ref, ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
    }
    return result;
...
template <typename fp>
bool check_or_un_gbr_accuracy(oneapi::mkl::generate vect, int64_t m, int64_t n, int64_t k,  const fp* Q, int64_t ldq) {
    bool result = true;
    ...
    if ( vect == oneapi::mkl::generate::Q ) {
        ...
        int64_t ldqq = cols_Q;
        reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::conjtrans, cols_Q, rows_Q, 1.0, Q, ldq, 0.0, QQ.data(), ldqq);
        ...
        int64_t ldpp = rows_P;
        reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, rows_P, cols_P, 1.0, P, ldp, 0.0, PP.data(), ldpp);
...
bool check_or_un_gqr_accuracy(int64_t m, int64_t n, const fp* Q, int64_t ldq) {
    ...
    reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::conjtrans, n, m, 1.0, Q, ldq, 0.0, QQ.data(), n);
...
bool check_or_un_gtr_accuracy(int64_t n, const fp* Q, int64_t ldq) {
    ...
    /* | I - Q'Q | < m O(eps) */
    std::vector<fp> QQ(n*n);
    reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::conjtrans, n, n, 1.0, Q, ldq, 0.0, QQ.data(), n);
    hermitian_to_full(oneapi::mkl::uplo::upper, n, QQ, n);
...
template <typename fp>
bool check_geqrf_accuracy(const fp* A, const fp* A_initial, const fp* tau, int64_t m, int64_t n, int64_t lda) {
    bool result = true;
    /* |A - Q R| < |A| O(eps) */
    std::vector<fp> R(m*n);
    int64_t ldr = m;
    ...
    auto info = reference::or_un_mqr(oneapi::mkl::side::left, oneapi::mkl::transpose::nontrans, m, n, std::min(m,n), A, lda, tau, R.data(), ldr);
    if( 0 != info) {
    ...
    int64_t ldqq = m;
    reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, m, m, 1.0, Q.data(), ldq, 0.0, QQ.data(), ldqq);
...
template <typename fp>
bool check_getrf_accuracy(int64_t m, int64_t n, const fp* A, int64_t lda, const int64_t* ipiv, const fp* A_initial) {
    using fp_real = typename complex_info<fp>::real_type;
    ...
    if (m < n) {
        ...
        reference::trmm(oneapi::mkl::side::left, oneapi::mkl::uplo::lower, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::unit, m, n, 1.0, A, lda, residual.data(), m);
    } else {
        reference::lacpy(oneapi::mkl::uplo::lower, m, n, A, lda, residual.data(), m);
        for (int64_t diag = 0; diag < n; diag++)
            residual[diag + diag*m] = 1.0;
        reference::trmm(oneapi::mkl::side::right, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::nonunit, m, n, 1.0, A, lda, residual.data(), m);
    }
    ...
    bool result = rel_err < threshold;
    if(!result){
        snprintf(global::buffer.data(), global::buffer.size(), "\t| L * U - A | / ( |A| * min(m,n) * ulp ) = |%e| / (|%e| %d * %e) = %e" , norm_residual, norm_A, static_cast<int>(std::min(m,n)), ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
    }
...
template <typename fp>
bool check_potrf_accuracy(const fp* init, const fp* sol, oneapi::mkl::uplo uplo, int64_t n, int64_t lda) {
    using fp_real = typename complex_info<fp>::real_type;
    ...
    bool lower = (uplo == oneapi::mkl::uplo::upper); // lower for row-major (which is this source) is upper for column major
    bool result = true;
    ...
    for (int64_t i = 0; i < n; i++) {
        for (int64_t j = 0; j < i+1; j++) {
            ...
            if (error > ((n+1)*eps/(1-(n+1)*eps)) * std::sqrt(std::abs(init[i*lda+i]*init[j*lda+j]))) {
                result = false;
            }
...
bool check_getri_accuracy(int64_t n, fp* A, int64_t lda, int64_t* ipiv, fp* A_initial) {
    ...
    reference::gemm(oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, n, n, n, -1.0, A, lda, A_initial, lda, 1.0, residual.data(), n);
    ...
    bool result = rel_err < threshold;
    if(!result){
        snprintf(global::buffer.data(), global::buffer.size(), "\t| I - inv(A) A | / ( |A| |inv(A)| n ulp ) = |%e| / ( |%e| |%e| %d * %e ) = %e" , norm_residual, norm_A, norm_invA, static_cast<int>(n), ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
    }
    ...
    reference::laset('A', n, n, 0.0, 1.0, residual.data(), n);
    reference::gemm(oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, n, n, n, -1.0, A_initial, lda, A, lda, 1.0, residual.data(), n);
...
template <typename fp>
bool check_getrs_accuracy(oneapi::mkl::transpose transa, int64_t n, int64_t nrhs, fp* A, int64_t lda, int64_t* ipiv, fp* B, int64_t ldb, fp* A_initial, fp* B_initial) {

    using fp_real = typename complex_info<fp>::real_type;

    // Compute A*X - B. Store result in B_initial
    reference::gemm(transa, oneapi::mkl::transpose::nontrans, n, nrhs, n, -1.0, A_initial, lda, B, ldb, 1.0, B_initial, ldb);
    ...
    bool result = rel_err < threshold;
    if(!result){
        snprintf(global::buffer.data(), global::buffer.size(), "\t| AX - B | / ( |A| |X| n ulp ) = |%e| / ( |%e| |%e| %d * %e ) = %e" , norm_residual, norm_A, norm_B, static_cast<int>(n), ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
    }
...
template <typename fp>
bool check_potrs_accuracy(oneapi::mkl::uplo uplo, int64_t n, int64_t nrhs, fp* A, int64_t lda, fp* B, int64_t ldb, fp* A_initial, fp* B_initial) {
    using fp_real = typename complex_info<fp>::real_type;

    hermitian_to_full(uplo, n, A_initial, lda);
    // Compute A*X - B. Store result in B_initial
    reference::gemm(oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, n, nrhs, n, -1.0, A_initial, lda, B, ldb, 1.0, B_initial, ldb);
...
bool check_gerqf_accuracy(fp* A, fp* A_initial, fp* tau, int64_t m, int64_t n, int64_t lda) {
    ...
    if (m >= n) {
        ...
        reference::laset(oneapi::mkl::uplo::lower, n-1, n-1, 0.0, 0.0, R.data()+((m-n+1)+0*ldr), ldr);

        std::vector<fp> Q(lda*n);
        int64_t ldq = n;
        reference::lacpy('A', n, n, A+((m-n)+0*lda), lda, Q.data(), ldq);

        auto info = reference::or_un_mrq(oneapi::mkl::side::right, oneapi::mkl::transpose::nontrans, m, n, std::min(m,n), Q.data(), ldq, tau, R.data(), ldr);
        ...
        reference::lacpy(oneapi::mkl::uplo::upper, m, m, A+(0+(n-m)*lda), lda, R.data()+(0+(n-m)*ldr), ldr);
        ...
        for (int64_t i = 0; i < std::min(m,n); i++)
            tau2[n-m+i] = tau[i];
        auto info = reference::or_un_mrq(oneapi::mkl::side::right, oneapi::mkl::transpose::nontrans, m, n, n, Q.data(), ldq, tau2.data(), R.data(), ldr);
        if( 0 != info) {
            global::log << "\treference ormqr/unmqr failed with info = " << info << std::endl;
            return false;
        }
    ...
    reference::sy_he_rk(oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, std::min(m,n), n, 1.0, Q.data(), ldq, 0.0, I.data(), ldi);
...
template <typename fp>
bool check_trtrs_accuracy(oneapi::mkl::uplo uplo, oneapi::mkl::transpose trans, oneapi::mkl::diag diag, int64_t n, int64_t nrhs, fp* A, int64_t lda, const fp* B, int64_t ldb, const fp* B_initial) {
    using fp_real = typename complex_info<fp>::real_type;
    ...
    int64_t ldr = n;
    reference::gemm(trans, oneapi::mkl::transpose::nontrans, n, nrhs, n, 1.0, A, lda, B, ldb, 0.0, residual.data(), ldr);
    ...
    bool result = rel_err < threshold;
    if(!result) {
        snprintf(global::buffer.data(), global::buffer.size(), "\t|Ax - b| / (|A| |x| cond(A) eps) = |%e| / (|%e| |%e| %e * %e) = %e\n", norm_residual, norm_A, norm_x, cond_A, ulp, rel_err);
        global::log << global::buffer.data();
        global::log << "\tthreshold = " << threshold << std::endl;
