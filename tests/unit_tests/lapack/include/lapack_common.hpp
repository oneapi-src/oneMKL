...
template <typename fp> fp get_real(fp val) { return val; }
template <typename fp> std::complex<fp> get_real(std::complex<fp> val) { return val.real(); }

template <typename fp> fp get_conj(fp val) { return val; }
template <typename fp> std::complex<fp> get_conj(std::complex<fp> val) { return std::conj(val); }
...
template <> inline std::complex<float> rand_scalar(uint64_t& seed) { 
    return std::complex<float>(rand_scalar<float>(seed), rand_scalar<float>(seed));
}
template <> inline std::complex<double> rand_scalar(uint64_t& seed) { 
    return std::complex<double>(rand_scalar<double>(seed), rand_scalar<double>(seed));
}

template <typename vec>
void rand_matrix(uint64_t& seed, oneapi::mkl::transpose trans, int64_t m, int64_t n, vec &M, int64_t ld, int64_t offset = 0) {
...
template <typename vec>
void rand_symmetric_matrix(uint64_t& seed, oneapi::mkl::uplo uplo, int64_t n, vec &M, int64_t ld, int64_t offset = 0) {
    using fp = typename vec::value_type;
...
template <typename vec>
void rand_hermitian_matrix(uint64_t& seed, oneapi::mkl::uplo uplo, int64_t n, vec &M, int64_t ld, int64_t offset = 0) {
    using fp = typename vec::value_type;
    using fp_real = typename complex_info<fp>::real_type;
...
template <typename vec>
void rand_pos_def_matrix(uint64_t& seed, oneapi::mkl::uplo uplo, int64_t n, vec &M, int64_t ld, int64_t offset = 0) {
    using fp = typename vec::value_type;
    using fp_real = typename complex_info<fp>::real_type;
...
template <typename mem_T, is_buffer_type<mem_T> = nullptr>
void host_to_device_copy(sycl::queue queue, typename mem_T::value_type* source, mem_T dest, size_t count) {
    queue.submit([&](sycl::handler& cgh) {
        auto dest_accessor = dest.template get_access<sycl::access::mode::discard_write>(cgh, sycl::range<1>(count));
...
template <typename mem_T, is_buffer_type<mem_T> = nullptr>
void device_to_host_copy(sycl::queue queue, mem_T source, typename mem_T::value_type* dest, size_t count) {
    queue.submit([&](sycl::handler& cgh) {
        auto source_accessor = source.template get_access<sycl::access::mode::read>(cgh, sycl::range<1>(count));
