...
int test(device* dev, oneapi::mkl::layout layout, int N, int incx) {
    ...
    try {
        ...
        switch (layout) {
            case oneapi::mkl::layout::column_major:
                TEST_RUN_CT_SELECT(main_queue, oneapi::mkl::blas::column_major::iamax,
                            N, x.data(), incx, result_p, dependencies);
                break;
            case oneapi::mkl::layout::row_major:
                TEST_RUN_CT_SELECT(main_queue, oneapi::mkl::blas::row_major::iamax,
                            N, x.data(), incx, result_p, dependencies);
                break;
            default: break;
        }
