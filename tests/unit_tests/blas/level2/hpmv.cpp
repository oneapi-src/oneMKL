...
int test(device* dev, oneapi::mkl::layout layout, oneapi::mkl::uplo upper_lower, int n,
         fp alpha, fp beta, int incx, int incy) {
    // Prepare data.
    ...
    try {
        ...
        switch (layout) {
            case oneapi::mkl::layout::column_major:
                TEST_RUN_CT(main_queue, oneapi::mkl::blas::column_major::hpmv,
                            (main_queue, upper_lower, n, alpha, A_buffer, x_buffer, incx, beta,
                             y_buffer, incy));
                break;
            case oneapi::mkl::layout::row_major:
                TEST_RUN_CT(main_queue, oneapi::mkl::blas::row_major::hpmv,
                            (main_queue, upper_lower, n, alpha, A_buffer, x_buffer, incx, beta,
                             y_buffer, incy));
                break;
            default: break;
