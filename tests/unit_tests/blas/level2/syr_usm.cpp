...
int test(const device &dev, oneapi::mkl::layout layout, oneapi::mkl::uplo upper_lower, int n,
    ...
    try {
        ...
        switch (layout) {
            case oneapi::mkl::layout::column_major:
                TEST_RUN_CT(main_queue, oneapi::mkl::blas::column_major::syr,
                            (main_queue, upper_lower, n, alpha, x.data(), incx, A.data(), lda,
                             dependencies));
                break;
            case oneapi::mkl::layout::row_major:
                TEST_RUN_CT(main_queue, oneapi::mkl::blas::row_major::syr,
                            (main_queue, upper_lower, n, alpha, x.data(), incx, A.data(), lda,
                             dependencies));
                break;
            default: break;
